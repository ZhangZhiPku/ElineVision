...
def build_network(input_batch_shape=(_x_dimension, ), n_layers=4, use_bias=True):
    ...
    layers = [tf.keras.layers.Dense(units=32, activation='tanh', use_bias=use_bias) for i in range(n_layers)]
...
if __name__ == '__main__':
    ...
    _model, _layers = build_network(use_bias=_use_bias)

    _rxs, _rys, _bxs, _bys = [], [], [], []
    for _layer in _layers:
        if _use_bias:
            _weights_shape = np.array(_layer.get_weights()[: -1]).shape
            _rxs.append(np.random.normal(size=_weights_shape))
            _rys.append(np.random.normal(size=_weights_shape))

            _weights_shape = np.array(_layer.get_weights()[-1]).shape
            _bxs.append(np.random.normal(size=_weights_shape))
            _bys.append(np.random.normal(size=_weights_shape))
        else:
    ...
    for _x_multiplier in _multipliers:
        for _y_multiplier in _multipliers:
            for _rx, _ry, _bx, _by, _layer in zip(_rxs, _rys, _bxs, _bys, _layers):
                if _use_bias:
                    _layer.set_weights(
                        np.add(_rx * _x_multiplier, _ry * _y_multiplier) + \
                        np.add(_bx * _x_multiplier, _by * _y_multiplier)
                    )
                else:
